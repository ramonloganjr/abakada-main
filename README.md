<div align="center">
  ...
  <p>A curated, multilingual directory of 1,294 free and open-source productivity tools<br/>for Filipino students, educators, scholars, and professionals.</p>
...
- 1,294 free and open-source tools across 45+ categories
...
- Includes the same impact stats shown in the deck (1,294 tools · 45+ categories · 10 paths · 4 languages)
...
- `sitemap.xml` regenerated on every build via `scripts/refresh-sitemap.js`
  - 1,320 URLs total: 15 static + 1 external (the Abakada Toolkit companion app) + 10 toolkits + 1,294 tools (the `noindex` `/progress` and `/bookmarks` are intentionally excluded)
...
| Static prerendering | 1,363 HTML shells written by `scripts/prerender-shells.js` |
...
│   ├── assets/
│   │   ├── data/
│   │   │   ├── tools.json                # All tool data (1,294 entries)
...
│   ├── prerender-shells.js               # postbuild: writes 1,363 static HTML shells
...
3. `postbuild` →
   - `scripts/prerender-shells.js` — writes 1,363 static HTML shells (59 static across 4 locales + 10 toolkits + 1,294 tools)
...
- `dist/<route>/index.html`: 1,363 prerendered route shells (plus the root shell = 1,364 HTML files total)
...
- **Prerendered routes win** — Vercel checks the filesystem before applying rewrites, so each of the 1,363 prerendered shells serves its own `<title>`, meta description, canonical and JSON-LD instead of the generic app shell. This is what keeps the SEO/AEO/GEO layer intact; verify it on a preview with `curl -s <url>/tools/vlc | grep '<title>'`.
