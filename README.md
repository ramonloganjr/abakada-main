<div align="center">
  ...
  <p>A curated, multilingual directory of 1,296 free and open-source productivity tools<br/>for Filipino students, educators, scholars, and professionals.</p>
...
| Metric | Value |
|:---|:---|
| Tools | 1,296 (manually curated) |
| Categories | 45+ |
| Learning paths | 10 |
| Languages | 4 (English, Tagalog, Ilokano, Bisaya) — 983 keys each, full parity |
| Prerendered HTML routes | 1,365 (59 static pages across 4 locales + 10 toolkits + 1,296 tools) |
...
- 1,296 free and open-source tools across 45+ categories
...
- Includes the same impact stats shown in the deck (1,296 tools · 45+ categories · 10 paths · 4 languages)
...
- `sitemap.xml` regenerated on every build via `scripts/refresh-sitemap.js`
  - 1,322 URLs total: 15 static + 1 external (the Abakada Toolkit companion app) + 10 toolkits + 1,296 tools (the `noindex` `/progress` and `/bookmarks` are intentionally excluded)
...
| Static prerendering | 1,365 HTML shells written by `scripts/prerender-shells.js` |
...
│   ├── assets/
│   │   ├── data/
│   │   │   ├── tools.json                # All tool data (1,296 entries)
...
│   ├── prerender-shells.js               # postbuild: writes 1,365 static HTML shells
...
3. `postbuild` →
   - `scripts/prerender-shells.js` — writes 1,365 static HTML shells (59 static across 4 locales + 10 toolkits + 1,296 tools)
...
- `dist/<route>/index.html`: 1,365 prerendered route shells (plus the root shell = 1,366 HTML files total)
...
- **Prerendered routes win** — Vercel checks the filesystem before applying rewrites, so each of the 1,365 prerendered shells serves its own `<title>`, meta description, canonical and JSON-LD instead of the generic app shell. This is what keeps the SEO/AEO/GEO layer intact; verify it on a preview with `curl -s <url>/tools/vlc | grep '<title>'`.
